			...
			<table width="600px">
				...
				<c:forEach items="${posts}" var="post">
					...
					<tr>
						...
						<td>${post.title}</td>
						<td>${post.body}</td>
						<td align="center"><form:form action="${url}" method="GET">
								<input alt="Exibir Post"
									src="<c:url value=
                 "/static/images/show.png"/>"
									title="Exibir Post" type="image" value="Exibir Post" />
							</form:form></td>
						<td align="center"><form:form action="${url}/form"
								method="GET">
								<input alt="Atualizar Post"
									src="<c:url value=
                 "/static/images/update.png"/>"
									title="Atualizar Post" type="image" value="Atualizar Post" />
							</form:form></td>
						<td align="center"><form:form action="${url}" method="DELETE">
								<input alt="Excluir Post"
									src="<c:url value=
                 "/static/images/delete.png"/>"
									title="Excluir Post" type="image" value="Excluir Post" />
							</form:form></td>
					</tr>
				</c:forEach>
			</table>
